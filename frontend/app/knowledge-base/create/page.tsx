            ...
            <Card className="max-w-2xl mx-auto">
              <div className="text-center space-y-4">
                ...
                <div className="flex justify-center space-x-3 pt-4">
                  <Button onClick={handleGenerateTags} color="purple" disabled={generatingTags}>
                    {generatingTags ? (
                      <>
                  ...
                  <Button onClick={handleViewArticle} className="bg-orange-600 hover:bg-orange-700 focus:ring-orange-500">
                    View Article
                  </Button>
                </div>
              </div>
        ...
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              <Button onClick={handleBack} color="gray" className="flex items-center">
                <ArrowLeft className="h-4 w-4 mr-2" />
                Back
              </Button>
              <div>
                <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Create New Article</h1>
                <p className="text-sm text-gray-500 dark:text-gray-400">Add a new article to the knowledge base</p>
              </div>
            </div>
          </div>
          ...
          <Card className="max-w-4xl">
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                ...
                <TextInput id="title" placeholder="Enter article title..." value={title} onChange={(e) => setTitle(e.target.value)} className="mt-2" required />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Select id="category" value={selectedCategoryId} onChange={(e) => setSelectedCategoryId(e.target.value)} className="mt-2" required>
                ...
                <div>
                  ...
                  <Select id="subcategory" value={selectedSubcategoryId} onChange={(e) => setSelectedSubcategoryId(e.target.value)} className="mt-2" disabled={!selectedCategoryId} required>
                    <option value="">Select a subcategory</option>
                    {availableSubcategories.map((subcategory) => (
                      <option key={subcategory.id} value={subcategory.id}>{subcategory.name}</option>
                    ))}
                  </Select>
                  {!selectedCategoryId && <p className="text-xs text-gray-500 mt-1">Please select a category first</p>}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="mt-2">
                  <RichTextEditor content={content} onChange={setContent} placeholder="Write your article content here..." className="min-h-[400px]" />
                </div>
              </div>
