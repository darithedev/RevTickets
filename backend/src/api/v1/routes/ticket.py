...
async def ai_reassign_ticket(ticket_id: PydanticObjectId, current_user: User = Depends(get_current_agent_user)):
    ...
    response = AIAssignmentResponse(success=result["success"], assignment_reason=result["assignment_reason"], confidence=result.get("confidence", 0))
    if result["success"] and result["agent"]:
        response.ticket = await TicketService.get_ticket(ticket_id)
